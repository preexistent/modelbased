...
class MultiAgentEnv(gym.Env):
    ...
    def step(self, action_n, use_rvo):
        obs_n = []
        reward_n = []
        done_n = []
        info_n = {'n': []}
        ...
        for i, agent in enumerate(self.agents):
            ...
            if use_rvo is not None and use_rvo[i] == True :
                if self.world.collision(agent):
                    ...
                    for j in list_marks:
                        if j < self.world.num_goals:
                            dist = np.sqrt(np.sum(np.square(agent.state.p_pos - self.world.landmarks[j].state.p_pos)))
                            if dist < least_dist:
                                idx = j
                    goal_pose = self.world.landmarks[idx]
                    list_marks.remove(idx)
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        agent.action.rot = np.zeros(self.world.dim_rot)
        agent.action.c = np.zeros(self.world.dim_c)
        # process action
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                ...
                # process discrete rotation
                if action[0] == 5: agent.action.rot[0] = -1.0
                if action[0] == 6: agent.action.rot[0] = +1.0
                ...
                if self.discrete_action_space:
                    ...
                    agent.action.rot += action[0][5] - action[0][6]
                else:
                    agent.action.u = action[0][0:5]
                    agent.action.rot = [action[0][0]]+[action[0][5:7]]
