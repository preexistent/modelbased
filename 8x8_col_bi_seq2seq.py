...
def train(model, iterator, optimizer, criterion, clip):
    ...
    for i, (src, trg) in enumerate(iterator):
        ...
        loss = criterion(output.view([-1,output.shape[2]]), trg.reshape(8*128))
...
def evaluate(model, iterator, criterion):
    ...
    with torch.no_grad():
        for i, (src, trg) in enumerate(iterator):
            ...
            for vec in range(trg.size(1)):
                correct += (predicted[:, vec] == (trg[:, vec])).all().item()
            predicted_dis += computeDistance(torch.transpose(torch.transpose(src, 0, 1), 1, 2), torch.transpose(predicted, 0, 1))
            optimal_dis += computeDistance(torch.transpose(torch.transpose(src, 0, 1), 1, 2), torch.transpose(trg, 0, 1))
            if i+1%10 == 0:
...
def loading_data(num_robots):
    ...
    distanceMatrices = pandas.read_csv('../data/8x8_seqdata/distanceMatrices.csv',
                                       header=None,
                                       nrows=200000,
                                       sep=' ',
                                       dtype='float')
    distanceMatrices = distanceMatrices.values
    assignmentMatrices = pandas.read_csv('../data/8x8_seqdata/assignmentMatrices.csv',
                                       header=None,
                                       nrows=200000,
                                       sep=' ',
                                       dtype='float')
    ...
    distanceMatrices = np.transpose(distanceMatrices.reshape(N, num_robots, num_robots), [0, 2, 1]) #col representation
...
if training:
    N_EPOCHS = 100
    CLIP = 10
    SAVE_DIR = 'models/8x8_col_bidirectional'
    ...
    import os
    if not os.path.exists('results/8x8col_bi'):
        os.makedirs('results/8x8col_bi')
    with open('results/8x8col_bi/optimal_dist_col_bidirectional.csv', 'wb') as f:
        np.savetxt(f,
                   np.asarray(optimal_train),
                   newline='\n',
                   fmt='%f')
    with open('results/8x8col_bi/row_bipredicted_dist_col_bidirectional.csv', 'wb') as f:
        np.savetxt(f,
    ...
    print(test_acc_list)
    with open('results/8x8col_bi/test_accurracy_col_bidirectional.csv', 'wb') as f:
        np.savetxt(f,
